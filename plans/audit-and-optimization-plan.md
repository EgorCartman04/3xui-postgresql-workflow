...
1. Задача 1 (CRITICAL — блокер UI SNI Manager): `import sqlite3`.
2. Задача 2 (неверный IP — две строки в update-3x-ui.sh).
3. Задача 3 (no_log — безопасность, 4 файла).
...
6. **Постдеплой-проверки:**
   - SNI Manager UI жив (задача 1): туннель + `GET /healthz` и `/api/status` — больше не
     падает с NameError. Локально: `./scripts/tunnel.sh status`.
